...
void dtun_dm_sig_local_info(tDTUN_DEVICE_SIGNAL *msg)
{
    ...
    if( retval < 0 )
    //CHG-s by Yingwei: [Feature-BRANDY_F10160]Modify for set Bluetooth device name
        //strcpy( str, DTUN_DEFAULT_DEV_NAME );
    {
        memset(str, 0, DTUN_MAX_DEV_NAME_LEN);
        property_get("ro.product.model", str, "");
    }
    //CHG-e by Yingwei: [Feature-BRANDY_F10160]Modify for set Bluetooth device name
