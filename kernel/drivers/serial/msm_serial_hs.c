...
static int msm_hs_ioctl(struct uart_port *port, unsigned int cmd, unsigned long arg)
{
    if (!(cmd == 0x8003 || cmd == 0x8004 || cmd == 0x8005))
        return -ENOIOCTLCMD;

    if (bluesleep_ops && bluesleep_ops->bluesleep_event)
        return bluesleep_ops->bluesleep_event(port,UART_BLUESLEEP_IOCTL,cmd,arg);
    else
        return -1;
}
...
static struct uart_ops msm_hs_ops = {
	...
	.ioctl = msm_hs_ioctl,//yangyufeng for bluesleep
};
