...
static void usb_chg_stop(struct work_struct *w)
{
	...
	if (temp == USB_CHG_TYPE__SDP)
		hsusb_chg_vbus_draw(0);			//disable hsusb rpc for debug usb pulg out no sleep
}
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	if (maxpower > 0)
		hsusb_chg_vbus_draw(maxpower);		//disable hsusb rpc for debug usb pulg out no sleep 
	 ...
	 * */
	if (temp == USB_CHG_TYPE__WALLCHARGER) {
#if 0	/*jrd change for usb and wall charger detect*/
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ui->chg_current = mA;
	hsusb_chg_vbus_draw(mA);	//disable hsusb rpc for debug usb pulg out no sleep	
