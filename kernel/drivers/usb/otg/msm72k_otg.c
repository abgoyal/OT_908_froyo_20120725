...
#include <linux/wakelock.h>		/*debug usb pull out sleep*/
#define PRINT_CLK_LOG	0		/*add for debug pclk and cclk*/
/*add finish*/

#define MSM_USB_BASE	(dev->regs)
#define is_host()	0 //((OTGSC_ID & readl(USB_OTGSC)) ? 0 : 1) //jrd modify for sleep
...
static int msm_otg_set_clk(struct otg_transceiver *xceiv, int on)
{
	...
	if (on)
		...
		{
                #if PRINT_CLK_LOG
                printk("%s, line%d, disable hs_clk \n", __FUNCTION__, __LINE__);
		#endif
...
/*
		                #if PRINT_CLK_LOG	
				printk("%s, enter pclk_required_during_lpm line%d, enable hs_pclk \n", __FUNCTION__, __LINE__);
				#endif
...
/*
		if (dev->pdata->pclk_required_during_lpm)
			{
			        #if PRINT_CLK_LOG
        	        	printk("%s, line%d, disable hs_pclk \n", __FUNCTION__, __LINE__);
	        	        #endif
...
static void msm_otg_start_host(struct otg_transceiver *xceiv, int on)
{
	...
	if (dev->pdata->pclk_required_during_lpm) {
		if (on)
			{
	                	#if PRINT_CLK_LOG
		                printk("%s, line%d, enable hs_pclk \n", __FUNCTION__, __LINE__);
        		        #endif
				clk_enable(dev->hs_pclk);
			}
		else
...
/*add finish*/
	...
	if (dev->hs_cclk)
		{
                #if PRINT_CLK_LOG
                printk("%s, line%d, disable hs_cclk \n", __FUNCTION__, __LINE__);
                #endif
...
static int msm_otg_resume(struct msm_otg *dev)
{
	...
	if (dev->hs_cclk)
		{
                #if PRINT_CLK_LOG
                printk("%s, line%d, enable hs_cclk \n", __FUNCTION__, __LINE__);
                #endif
...
static void otg_reset(struct otg_transceiver *xceiv)
{
	...
	if (dev->pdata->phy_reset)
		...
		{
        #if PRINT_CLK_LOG
        printk("%s, line%d, msm_otg_phy_reset \n", __FUNCTION__, __LINE__);
        #endif
