...
const int ili9841_bk_intensity[8] = {0,14,15,16,4,3,2,1};
//unsigned int ili9841_bk_intensity[7] = {0,1,2,3,4,5,6};
...
static int ili9481_init(void)
{
	...
	ili9481_reg_data(0x42);	//PON=1,BT2-0=2
...
static int mc8416_control(const int step)
{
	...
	nr_pulse = ili9841_bk_intensity[step];
	if(nr_pulse>16)
		nr_pulse = 16;
...
static int lcdc_ili9841_bk_setting(const int step)
{
	if (step == 0)
...
{
	if (ili9481_state.display_on==TRUE)
		lcdc_ili9841_bk_setting(mfd->bl_level);
}
...
static int __init ili9481_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
