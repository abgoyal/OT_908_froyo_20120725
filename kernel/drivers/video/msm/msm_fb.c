...
// { Blue screen on painc, xuxian 20110217
#include "font5x12.h"
char dump_start = 0;
					...
					enum led_brightness value)
{
	...
	int bl_lvl;
//{ xuxian, change to avoid screen flash towice when off->on, 20110306
printk("brightness value is %d\n", value);
	if (value > 255)
		{
			bl_lvl = (2 * (value-255) * mfd->panel_info.bl_max + 255)
				/(2 * 255);
			bl_lvl+=255;
		}else{
// }
	...
	bl_lvl = (2 * value * mfd->panel_info.bl_max + 255)
		/(2 * 255);
...
//{ xuxian, change to avoid screen flash towice when off->on, 20110306
	if(bkl_lvl > 255)
		{
		printk("xuxian debug : back light levle is more than 255, it is just fine...........\n ");
		if ((pdata) && (pdata->set_backlight)) {
			u32 new_lvl = bkl_lvl -255;
		down(&mfd->sem);
		if (new_lvl != mfd->bl_level) {
			mfd->bl_level = new_lvl;
			if(is_first_boot){
...
#define FGCOLOR 0xFFFF	//white
#define BGCOLOR 0x001F	//blue
...
void clear_console(void)
{
    ...
    unsigned count = LCD_MAX_X * LCD_MAX_Y;
    cx = 0;
    cy = 0;
    while(count--) *bits++ = BGCOLOR;
}
...
static void clear_line(void)
{
	...
	bits = (unsigned short *)(info->screen_base);
	bits += (cy*LCD_MAX_X*12);

	for ( row = 0 ; row < LCD_MAX_X; row++ )
...
void console_putc(unsigned c)
{
	...
	pixels = (unsigned short *)(info->screen_base);
    ...
    // fix cy in the same position
	drawglyph( pixels + cy * 12 * LCD_MAX_X + cx * 6, FGCOLOR,
		LCD_MAX_X, font5x12 + (c - 32) * 2);

    cx++;
    if(cx*6 < LCD_MAX_X) return;
...
void msm_fb_put_string( const char * fmt, ...)
{
	int i;
...
EXPORT_SYMBOL(msm_fb_put_string);
...
void lcd_dump( void )
{
	...
	mdelay(1000);
	for ( idx = 0 ; idx < dump_buffer_length ; idx++ )
	{
		console_putc( dump_buffer[idx] );
	}

	mdelay(4000);

	printk( "%s", dump_buffer );
#endif
}

EXPORT_SYMBOL(lcd_dump);
//}
